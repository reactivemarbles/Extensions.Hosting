...
/// <summary>Provides an IHostedService implementation that ensures only a single instance of the application runs by acquiring a named mutex at startup.</summary>
...
internal sealed class MutexLifetimeService(ILogger<MutexLifetimeService> logger, IHostEnvironment hostEnvironment, IHostApplicationLifetime hostApplicationLifetime, IMutexBuilder mutexBuilder) : IHostedService
{
    ...
    /// <summary>Stores the host environment value.</summary>
    ...
    /// <summary>Stores the mutex builder value.</summary>
    private readonly IMutexBuilder _mutexBuilder = mutexBuilder ?? throw new ArgumentNullException(nameof(mutexBuilder));

    /// <summary>Stores the resource mutex value.</summary>
    private ResourceMutex? _resourceMutex;

    /// <summary>Initializes the resource mutex and starts the application asynchronously, handling scenarios where another instance may already be running.</summary>
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        _ = _hostApplicationLifetime.ApplicationStopping.Register(OnStopping);
        if (!_resourceMutex.IsLocked)
        {
            _mutexBuilder.WhenNotFirstInstance?.Invoke(_hostEnvironment, _logger);
            _applicationAlreadyRunning(_logger, _hostEnvironment.ApplicationName, null);
