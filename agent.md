...
- **Main solution:** `src/Extensions.Hosting.slnx`
...
```bash
git clone --recursive https://github.com/reactivemarbles/Extensions.Hosting.git
````
...
* Main file: `src/Extensions.Hosting.slnx`
...
dotnet build Extensions.Hosting.slnx -c Release
dotnet build Extensions.Hosting.slnx -c Release -warnaserror

dotnet clean Extensions.Hosting.slnx
```
...
dotnet test --project tests/Extensions.Hosting.Tests/Extensions.Hosting.Tests.csproj
...
dotnet test --solution Extensions.Hosting.slnx -- --output Detailed
dotnet test --solution Extensions.Hosting.slnx --coverage --coverage-output-format cobertura -- --report-trx --output Detailed

# List tests
dotnet test --project tests/Extensions.Hosting.Tests/Extensions.Hosting.Tests.csproj -- --list-tests

# Fail fast
dotnet test --solution Extensions.Hosting.slnx -- --fail-fast

# Limit parallelism if needed (even though repo defaults non-parallel)
dotnet test --solution Extensions.Hosting.slnx -- --maximum-parallel-tests 4
```
...
```powershell
# Single test
dotnet test --project tests/Extensions.Hosting.Tests/Extensions.Hosting.Tests.csproj -- --treenode-filter "/*/*/*/MyTestMethod"

# All tests in class
dotnet test --project tests/Extensions.Hosting.Tests/Extensions.Hosting.Tests.csproj -- --treenode-filter "/*/*/MyClassName/*"

# All tests in namespace
dotnet test --project tests/Extensions.Hosting.Tests/Extensions.Hosting.Tests.csproj -- --treenode-filter "/*/MyNamespace/*/*"

# Filter by property (e.g., Category)
dotnet test --solution Extensions.Hosting.slnx -- --treenode-filter "/*/*/*/*[Category=Integration]"
```
...
### Core Library (`src/Extensions.Hosting.SingleInstance/`)
...
* `Extensions.Hosting.ReactiveUI.Wpf/`, `Extensions.Hosting.ReactiveUI.WinUI/`, `Extensions.Hosting.ReactiveUI.Maui/`, `Extensions.Hosting.Wpf/`,
  `Extensions.Hosting.WinUI/`, `Extensions.Hosting.ReactiveUI.Winforms/`, `Extensions.Hosting.Winforms/`, etc.
...
* Prefer existing patterns in:
  ...
  * `src/tests/Extensions.Hosting.Tests/`
